...
То есть, когда создается функция, например, \texttt{def f(): pass}, то создается \emph{переменная} \texttt{f}, которая получает ссылку на \emph{объект функции} \texttt{f()}.
...
Фундаментальное правило: оператор всегда возвращает \emph{новый экземпляр} (кроме составных операторов, которые возвращают \texttt{self}). Иначе говоря, нельзя модифицировать \texttt{self}, а нужно создавать и возвращать новый экземпляр подходящего типа \cite[\strbook{405}]{ramalho:python-2016}.
