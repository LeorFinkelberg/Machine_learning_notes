...
Можно загрузить пакет с TestPyPI вместо PyPI указав специальный флаг \verb|--index-url|
\begin{lstlisting}[
style = bash,
numbers = none	
]
...
\subsection{Рабочий пример \texttt{setup.py}}
...
Базовые файл сборки позаимствован из репозитория \url{https://github.com/navdeep-G/setup.py}
\begin{lstlisting}[
title = {\sffamily setup.py},
style = ironpython,
numbers = none	
...
В \texttt{SourceFileLoader} передаем полное имя и путь до модуля, который нужно загрузить, например, так
\begin{lstlisting}[
style = ironpython,
numbers = none	
]
...
В конфигурационный файл \texttt{.pre-commit-config.yaml}
\begin{lstlisting}[
